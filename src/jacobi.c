...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if(nrhs!=7) {
        mexErrMsgIdAndTxt("MATAMG:gs:nrhs", "Usage : V = GS(IA,JA,AA,V,F,W,MAXITER)");
    }
    ...
    vnewr = mxCalloc(sizeofA, sizeof(double));
    vnewi = mxCalloc(sizeofA, sizeof(double));
    
    if ( aai == NULL )  /* Check complex matrix */ {
        if ( maxiter > 0 ) {
            for ( k = 1; k <= maxiter; k++ ) {
                iaim1 = ia[ 0 ];
                for ( i = 1; i < sizeofA + 1 ; i++ ) {
                    ...
                    if (dr == 0.0)
                        mexErrMsgIdAndTxt("MATAMG:jacobi:diag", "There is zero diagonal");
                    vnewr[ im1 ] = w * ( fr[ im1 ] - valr )/dr + ( 1 - w ) * vr[ im1 ];
                    iaim1 = iai;
                } /* end loop i */
        ...
        if ( vi == NULL ) {
            mexErrMsgIdAndTxt("MATAMG:jacobi:complex", "Initial vector must be complex");
        }
        else {
            if ( fi == NULL ) {
                if ( maxiter > 0 ) {
                    for ( k = 1; k <= maxiter; k++ ) {
                        ...
                        for ( i = 1; i < sizeofA + 1 ; i++ ) {
                            ...
                            if (dr == 0.0 & di == 0.0 )
                                mexErrMsgIdAndTxt("MATAMG:jacobi:diag", "There is zero diagonal");
                            bottom = dr*dr + di*di;
                            vnewr[ im1 ] = w * (( fr[ im1 ] - valr)*dr -  vali*di)/bottom + (1-w)*vr[ im1 ];
                            vnewi[ im1 ] = w * (( valr - fr[ im1 ] )*di - vali*dr)/bottom + (1-w)*vi[ im1 ];
                            iaim1 = iai;
                        }
                        vr = vnewr;
                        vi = vnewi;
                ...
                if ( maxiter > 0 ) {
                    for ( k = 1; k <= maxiter; k++ ) {
                        ...
                        for ( i = 1; i < sizeofA + 1 ; i++ ) {
                            ...
                            bottom = dr*dr + di*di;
                            vnewr[ im1 ] = w * (( fr[ im1 ] - valr)*dr + ( fi[ im1 ] - vali)*di)/bottom + (1-w)*vr[ im1 ];
                            vnewi[ im1 ] = w * (( valr - fr[ im1 ] )*di + ( fi[ im1 ] - vali)*dr)/bottom + (1-w)*vi[ im1 ];
                            iaim1 = iai;
                        }
                        vr = vnewr;
                        vi = vnewi;
